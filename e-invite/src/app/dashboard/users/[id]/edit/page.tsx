    ...
    const loadData = async () => {
      try {
        ...
        setSelectedInvitations(
          userData.invitations.map((ui) => ui.invitation.id)
        );
        ...
        if (invRes.ok) {
          ...
          if (Array.isArray(invData)) {
            setInvitations(
              invData.map((inv: { id: string; title: string; slug: string }) => ({
                id: inv.id,
                title: inv.title,
                slug: inv.slug,
              }))
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="mb-8">
        ...
        <h1 className="text-2xl sm:text-3xl font-bold text-white">Edit User</h1>
        <p className="text-gray-400 mt-1">Update user account details</p>
      </div>
